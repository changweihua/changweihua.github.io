...
commentabled: true
recommended: true
title: AgentFramework实现AI智能体Skill和工具动态管理和加载
description: AgentFramework实现AI智能体Skill和工具动态管理和加载
...
```mermaid
flowchart TD
    ChatClient[ChatClientAgent<br>（ChatOptions.Tools + AIContextProviders）]

    ChatClient -->|Tools| LeftGroup
    ChatClient -->|Skills| RightGroup

    subgraph LeftGroup [工具侧]
        direction TB
        ToolComponent[AIAgentTool / SkillService]
        ToolComponent --> SysTools[SysTools（静态工具）]
        ToolComponent --> IKevinTask[IKevinAITaskService 等]
        SysTools --> DB[AISkillToolManagementService（数据库）]
        IKevinTask --> DB
    end

    subgraph RightGroup [技能侧]
        direction TB
        SkillComponent[AgentSkillsProvider / Builder（AgentFramework）]
        SkillComponent --> FileSys[文件系统 Skill 目录]
    end
```
...
- 根据 agentId 查询绑定表 IAISkillToolBindIdService，获得该智能体允许使用的工具管理 ID。
- 通过 `AISkillToolManagementService.GetAllTools()` 过滤出这些 ID 对应的工具记录，提取 ClassMethod 名称。
- 调用 GetAITools 将方法名转化为 AITool 实例。
...
基于 AgentFramework，通过 数据库驱动 + 文件系统热加载 实现了 AI 智能体工具与技能的动态管理。其核心优势在于：
