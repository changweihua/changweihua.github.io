...
commentabled: true
recommended: true
title: 基于SpringBoot + Redis实现网站七天免密登录
description: 基于SpringBoot + Redis实现网站七天免密登录
...
```java
package net.cmono.webcore.util;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import net.cmono.webcore.dto.AutoLoginToken;
...
public class RedisTokenStore {
    ...
    public void storeToken(String token, AutoLoginToken autoLoginToken) {
        if (!StringUtils.hasText(token) || autoLoginToken == null || autoLoginToken.getUserId() == null) {
            throw new IllegalArgumentException("Token, AutoLoginToken and userId must not be null");
        }

        String key = TOKEN_PREFIX + token;
        String userKey = USER_TOKEN_PREFIX + autoLoginToken.getUserId();

        redisTemplate.opsForValue().set(key, autoLoginToken, TOKEN_EXPIRE_DAYS, TimeUnit.DAYS);
        redisTemplate.opsForValue().set(userKey, token, TOKEN_EXPIRE_DAYS, TimeUnit.DAYS);

        log.debug("Stored token for userId: {}", autoLoginToken.getUserId());
    }
    ...
    public AutoLoginToken getToken(String token) {
        ...
        if (value instanceof AutoLoginToken) {
            return (AutoLoginToken) value;
        }
        return null;
    }
    ...
    public void removeToken(String token, Long userId) {
        ...
        redisTemplate.delete(Arrays.asList(key, userKey));
        log.debug("Removed token for userId: {}", userId);
    }
}
```
...
public class TokenValidator {
    ...
    public boolean validateToken(String token, String deviceId, String userAgent, String ip) {
        if (!StringUtils.hasText(token) || !StringUtils.hasText(deviceId) || !StringUtils.hasText(userAgent)) {
            log.warn("Token validation failed: missing required parameters");
            return false;
        }
        ...
        if (!Objects.equals(storedToken.getDeviceId(), deviceId)) {
            log.warn("Token validation failed: device mismatch");
            return false;
        }

        if (!Objects.equals(storedToken.getUserAgent(), userAgent)) {
            log.warn("Token validation failed: User-Agent mismatch");
            return false;
        }

        if (storedToken.getIp() != null && !Objects.equals(storedToken.getIp(), ip)) {
            log.warn("Token validation failed: IP mismatch");
            return false;
        }

        log.debug("Token validation succeeded for userId: {}", storedToken.getUserId());
        return true;
    }
    ...
    public void refreshToken(String token) {
        if (!StringUtils.hasText(token)) {
            log.warn("Refresh token failed: token is empty");
            return;
        }

        AutoLoginToken storedToken = redisTokenStore.getToken(token);
        if (storedToken == null) {
            log.debug("Refresh token failed: token not found or expired");
            return;
        }

        redisTokenStore.storeToken(token, storedToken);
        log.debug("Token refreshed for userId: {}", storedToken.getUserId());
    }
}
```
