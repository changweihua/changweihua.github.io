...
recommended: true
title: 前端指纹技术是如何实现的？
description: Canvas、Audio、硬件API 核心原理解密
...
在讲实现之前，先纠正一个误区：设备指纹（Device Fingerprint）不是为了知道*你是张三*，而是为了知道 *这台设备是编号 9527*。
...
[👉 在线体验 Device Fingerprint](https://erpanomer.nurverse.com/tools/device-fingerprint)
...
光有 Canvas 和 Audio 还不够（因为同一型号的 iPhone 可能会完全一样）。这时候需要引入*动态硬件*特征来增加熵值。
