...
```mermaid
flowchart TB
    A[父组件]-->C1[provideValidation]
    C1--提供上下文-->D1[ValidationContext]
    A-->B1[子组件]
    B1-->C2[useValidation]
    C2--注入上下文-->D1
    B1--`onMounted`-->C3[addValidator]
    C3-->D2[validators数组]
    A--点击提交-->B2[validatorAll]
    B2--`遍历validators`-->C4{验证是否通过}
    C4--否-->D3[scrollToFirstError]
    C4--是-->D4[表单提交成功]
```
...
为了能够在多处复用校验逻辑，将具体的实现过程封装成 hooks，通过传入不同的 key，注入/获取不同的校验上下文。
...
生成一个校验上下文并注入，该组件的所有后代组件均可以使用该【校验上下文】，只需传递相同的 Key 值即可。
...
在页面挂载的时候，调用 `addValidator` 传入该组件自身的表单校验方法 `validate`，以及滚动的目标 DOM，同时 `validate` 中需要返回表单校验的结果。
