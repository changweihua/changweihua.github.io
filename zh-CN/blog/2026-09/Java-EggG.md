...
commentabled: true
recommended: true
title: 一行代码干翻 Java 反射？
description: EggG 流式反射调用让反射优雅到不可思议
...
| 方法名索引 | `ClassEgg` 内部预建方法名 `HashMap` 索引，`call()` 时 O(1) 定位候选方法 |
| 构造器参数快照 | `ConstrEgg` 预存参数别名数组，`create()` 时高性能匹配 |
