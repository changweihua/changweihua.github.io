...
description: 用过 5 个才算真正入门
date: 2026-01-06 13:30:00 
pageClass: blog-page-class
cover: /covers/vue.svg
---

> 如果你已经能够熟练地把 `ref/reactive`、`watch/computed`、`defineProps/defineEmits` 倒背如流，那么恭喜你，Vue3 的“入门课”已经通关。但 Vue3 在源码层埋了不少小而美的“隐藏关卡”，它们往往能在关键时刻把代码体积、性能、可维护性同时拉高一个档次。
> 
> 下面 10 个技巧，全部来自社区实战沉淀，每一个都附带真实业务场景 + 代码片段，看完直接就能搬进项目。
...
把只读展示层数据用 `shallowRef` 包起来，再把不会变动的配置用 `markRaw` 标记，Vue 会跳过深层 `Proxy` 创建，首屏渲染时间直接腰斩。
