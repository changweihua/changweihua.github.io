<template>
  <div class="public-gallery p-5">
    <div v-if="loading" class="flex items-center justify-center h-full min-h-24">
    ...
    <div v-else class="grid md:grid-cols-2 lg:grid-cols-4 images-grid">
      <div v-for="image in imageList" :key="image" class="image-card">
        <ImageZoom loading="lazy" class="medium-zoom-image" :src="`/images/${image}`" :alt="image" data-zoomable
...
.public-gallery {
  ...
  /* 第二层：稍微亮一点 */
  .surface-2 {
    background: light-dark(var(--surface-base-light),
  ...
  @supports (display: grid-lanes) {
    .images-grid {
      ...
      grid-template-columns:
        repeat(auto-fill, minmax(8rem, 1fr) minmax(16rem, 2fr)) minmax(8rem, 1fr);
      gap: 16px;
    }
  }
