<template>
  <ul id="cloudCon">
    <li :style="{ animation: item.styleCon }" v-for="(item, index) in hotWord" :key="`${currtIndex}_${index}`"
      :class="{ stop: item.isStop }" @click="handleMes(item.name)" @mouseenter="item.isStop = true"
