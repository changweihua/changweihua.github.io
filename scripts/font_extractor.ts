...
async function processDirectories(): Promise<void> {
  ...
  await readFileList(
    path.resolve(rootFolder, ".vitepress/components"),
    filesList,
  );
...
async function writeFile(fileArr: string[], fileName: string): Promise<void> {
    ...
    if (strArr?.length) {
      for (const item of strArr) {
        ...
        if (processedItem.length) {
          const str =
            processedItem.length >= 10
              ? `${processedItem.substring(0, 7)}...`
              : processedItem;
  ...
  const outputPath = path.resolve(
    rootFolder,
    `fonts-spider/temp/local/${fileName}.json`,
  );
...
async function processTemplate(): Promise<void> {
  console.log(
    chalk.blue(
      `[${new Date().toLocaleTimeString()}] 📦 填充字符到 index.html...`,
    ),
  );
  ...
  try {
    const templatePath = path.join(
      rootFolder,
      "fonts-spider/index.template.html",
    );
...
async function main(): Promise<void> {
  try {
    ...
    console.log(
      chalk.blue(`[${new Date().toLocaleTimeString()}] 🚀 开始文字提取流程...`),
    );
    ...
    console.log(
      chalk.green.bold(
        `✅  所有操作完成！ 耗时: ${totalSeconds.toFixed(2)} 秒`,
      ),
    );
